...
Uses the 10 CSVs exported by Section 45 of the notebook.
"""
...
GRADE_COLORS = {"A+": "#0B6E4F", "A": "#21A179", "B": "#F4A236", "C": "#E76F51", "D": "#D62828"}
GCC_COLORS = {
...
def require(*names):
    frames = {}
    missing = [n for n in names if (frames.update({n: load(n)}) or frames[n] is None)]
    # re-check
...
def fmt_usd(val, d=1):
    ...
    for threshold, suffix in [(1e12, "T"), (1e9, "B"), (1e6, "M"), (1e3, "K")]:
        if abs(val) >= threshold:
            return f"${val/threshold:.{d}f}{suffix}"
    return f"${val:,.0f}"
...
def backtest_metrics():
    ...
    return {"r2": r2, "mape_sig": mape_sig, "n": len(bt),
...
with st.sidebar:
    ...
    page = st.radio(
        "Navigate",
        ["Executive Summary", "Market Demand", "GCC Penetration",
         "Demand Forecasts", "Opportunity Finder"],
        label_visibility="collapsed",
    )
    st.divider()
    st.caption("Data: UN Comtrade · ITC Trade Map")
    st.caption("HS27/71/93/99 excluded")
...
if page == "Executive Summary":
    ...
    if bm:
        ...
        c1.metric("Forecast MAPE", f"{bm['mape_sig']:.1f}%", "on $1M+ series")
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    cmd_opts = (
        pen.groupby(["cmdCode", "commodity"])["world_demand"]
        .sum().reset_index().sort_values("world_demand", ascending=False)
    )
    ...
    fig3 = px.line(trend, x="year", y="d_B", markers=True,
                   labels={"d_B": "Import Demand (USD B)", "year": ""},
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    with col_l:
        ...
        fig2 = px.bar(high, y="label", x="penetration_pct", orientation="h",
                      labels={"penetration_pct": "GCC Penetration %", "label": ""},
    ...
    with col_r:
        ...
        if not gaps.empty:
            ...
            fig3 = px.bar(gaps, y="label", x="d_B", orientation="h",
                          labels={"d_B": "Import Demand (USD B)", "label": ""},
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    if not f.empty and not h.empty:
        ...
        fig.add_trace(go.Scatter(
            ...
            fill="toself", fillcolor="rgba(214,40,40,0.12)",
    ...
    fig2 = px.bar(top_fc, y="label", x="d_B", orientation="h",
                  labels={"d_B": "4-Year Forecast (USD B)", "label": ""},
                  color_discrete_sequence=["#D62828"])
...
# ═══════════════════════════════════════════════════════════════════════════
# OPPORTUNITY FINDER  —  GCC Country + Commodity → Top Destinations
# ═══════════════════════════════════════════════════════════════════════════
elif page == "Opportunity Finder":
    st.title("Opportunity Finder")
    st.markdown(
        "Select a **GCC exporter** and a **commodity** to see the best destination markets ranked by "
        "opportunity score. Score = demand (25%) + penetration gap (20%) + viability (20%) "
        "+ landing cost (15%) + ML growth (10%) + price (10%)."
    )

    opp = require("opportunity_rankings_full.csv")

    # --- FILTERS: GCC Country + Commodity ---
    gcc_list = sorted(opp["gcc_country"].unique())
    gcc_sel = st.selectbox("GCC Exporter", gcc_list)

    # Filter to selected GCC country first
    df_gcc = opp[opp["gcc_country"] == gcc_sel].copy()

    # Build commodity list for this GCC country, sorted by avg score
    cmd_scores = (
    ...
    sel_commodity_name = df["commodity"].iloc[0]

    # --- KPIs ---
    st.divider()
    k1, k2, k3, k4 = st.columns(4)
    k1.metric("Destination Markets", f"{df['dest_country'].nunique()}")
    ...
    if "demand_4y_total" in df.columns:
        k4.metric("Total 4Y Demand", fmt_usd(df["demand_4y_total"].sum()))

    # --- TOP DESTINATIONS BAR CHART ---
    st.divider()
    st.subheader(f"Top Destination Markets for {sel_commodity_name[:50]}")
    st.caption(f"GCC Exporter: **{gcc_sel}** · Ranked by opportunity score · Re-exports & saturated markets excluded")
    ...
    # Color by grade
    fig = px.bar(
        df_top, y="dest_country", x="opportunity_score", orientation="h",
        color="grade" if "grade" in df_top.columns else None,
        color_discrete_map=GRADE_COLORS,
        labels={"opportunity_score": "Opportunity Score", "dest_country": "", "grade": "Grade"},
        hover_data={c: True for c in ["penetration_pct", "demand_4y_total", "uv_mean", "ml_growth_prob"]
                    if c in df_top.columns},
    )
    fig.update_layout(
        margin=dict(t=10, l=10), height=max(350, top_n * 30),
    ...
    # Format columns without using .style
    ...
    if "Rationale" in table.columns:
        table["Rationale"] = table["Rationale"].str[:80]

    st.dataframe(table, use_container_width=True, hide_index=True, height=min(600, top_n * 40 + 50))

    # --- RATIONALE EXPANDER ---
    if "opportunity_rationale" in df_top.columns:
        with st.expander("📋 Full scoring rationale"):
    ...
    # --- BUBBLE CHART ---
    if all(c in df.columns for c in ["demand_4y_total", "pen_opportunity"]):
        st.divider()
        st.subheader("Demand vs Penetration Gap")
        st.caption("Top-right = sweet spot (high demand + low GCC presence). Bubble size = score.")
        bubble = df.head(30).copy()
        d_min, d_max = bubble["demand_4y_total"].min(), bubble["demand_4y_total"].max()
        bubble["demand_n"] = (bubble["demand_4y_total"] - d_min) / (d_max - d_min + 1e-9)

        fig2 = px.scatter(
            bubble, x="demand_n", y="pen_opportunity", size="opportunity_score",
            color="grade" if "grade" in bubble.columns else None,
            text="dest_country",
            color_discrete_map=GRADE_COLORS,
            labels={"demand_n": "Demand (normalised)", "pen_opportunity": "Entry Room (1 − pen%)"},
            size_max=25,
        )
        fig2.update_traces(textposition="top center", textfont_size=9)
        fig2.add_hline(y=0.5, line_dash="dash", line_color="grey", opacity=0.4)
        fig2.add_vline(x=0.5, line_dash="dash", line_color="grey", opacity=0.4)
        fig2.update_layout(margin=dict(t=10), height=480, showlegend=True)
        st.plotly_chart(fig2, use_container_width=True)
